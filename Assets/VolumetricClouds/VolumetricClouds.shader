Shader "Hidden/Sky/VolumetricClouds"
{
    Properties
    {
        ...
        [HideInInspector] _CloudMinimumLight("Cloud Minimum Light", Float) = 0.62
        [HideInInspector] _CloudMinimumLightColor("Cloud Minimum Light Color", Color) = (0.88, 0.9, 0.92, 1.0)
    ...
    SubShader
    {
        ...
        Pass
        {
        ...
        #endif
            {
                ...
                half cloudOpacity = saturate(1.0 - result.transmittance);
                half3 minimumScattering = _CloudMinimumLightColor.rgb * (_CloudMinimumLight * cloudOpacity);
                result.scattering = max(result.scattering, minimumScattering);
