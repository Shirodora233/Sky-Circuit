...
namespace SkyCircuit.Tutorial
{
    ...
    public sealed class SkyCircuitTutorialController : MonoBehaviour
    {
        ...
        private void EnsureCombatTutorialSession()
        {
            ...
            GameObject prefab = lanSession.RacePlayerPrefab;
            if (prefab == null)
            {
                Debug.LogWarning("Tutorial could not find the LAN race player prefab.");
                return;
            }

            Transform hostSpawn = lanSession.GetSpawnPoint(0);
            Transform clientSpawn = lanSession.GetSpawnPoint(1);
            GameObject hostPilot = InstantiatePilot(prefab, hostSpawn, "Tutorial Host Pilot");
            GameObject clientPilot = InstantiatePilot(prefab, clientSpawn, "Tutorial Client Pilot");
            if (hostPilot == null || clientPilot == null)
            {
                return;
            }

            tutorialPlayer = PreparePilot(hostPilot, 0, true);
            tutorialOpponent = PreparePilot(clientPilot, 1, false);
            ConfigureTutorialCamera(hostPilot);
            lanSession.StartOfflineTutorial(tutorialPlayer, tutorialOpponent);
        }

        private GameObject InstantiatePilot(GameObject prefab, Transform spawn, string objectName)
        {
            Vector3 position = spawn != null ? spawn.position : Vector3.zero;
            Quaternion rotation = spawn != null ? spawn.rotation : Quaternion.identity;
            GameObject pilot = Instantiate(prefab, position, rotation);
            pilot.name = objectName;
            return pilot;
        }

        private Competitor PreparePilot(GameObject pilot, int slot, bool isLocalPlayer)
        {
            ...
            else
            {
                ...
                SkyCircuitTutorialRoutePilot routePilot = pilot.GetComponent<SkyCircuitTutorialRoutePilot>();
                if (routePilot == null)
                {
                    routePilot = pilot.AddComponent<SkyCircuitTutorialRoutePilot>();
                }

                routePilot.Configure(controller, competitor, combatRoute);
            }

            return competitor;
        }

        private void ConfigureContrails(GameObject pilot, int slot)
        {
            Color cruise = slot == 1 ? lanSession.ClientTrailColor : lanSession.HostTrailColor;
            foreach (FlightContrailFeedback feedback in pilot.GetComponentsInChildren<FlightContrailFeedback>(true))
            {
                feedback.ConfigureColors(cruise, lanSession.BoostTrailColor);
                feedback.ClearExternalVisualState();
            }
        }

        private void ConfigureTutorialCamera(GameObject hostPilot)
        {
            if (hostPilot == null)
            {
                return;
    ...
    internal sealed class SkyCircuitTutorialRoutePilot : MonoBehaviour
    {
        ...
        public void Configure(SkyCircuitFlightController flightController, Competitor pilotCompetitor, BuoyRoute buoyRoute)
        {
        ...
        private void Update()
        {
            if (controller == null || competitor == null || route == null)
            {
                controller?.SetInput(FlightInputState.Neutral);
                return;
            }

            Transform target = route.GetTarget(competitor);
            Transform body = competitor.Body;
            ...
            RouteAIPilotSettings settings = competitor.Profile != null
                ? competitor.Profile.AiPilot.Validated()
            ...
            float turn = CalculateTurn(localTarget, settings.turnGain);
            float throttle = CalculateThrottle(distance, localTarget, turn, settings);
            float vertical = Mathf.Clamp(toTarget.y / settings.verticalRange, -1f, 1f);
            bool boost = settings.boostOnStraight && throttle > 0f && Mathf.Abs(turn) < 0.2f && Mathf.Abs(vertical) < 0.2f;

            controller.SetInput(new FlightInputState(throttle, turn, vertical, Vector2.zero, boost));
        }

        private float CalculateTurn(Vector3 localTarget, float turnGain)
        {
        ...
        private float CalculateThrottle(float distance, Vector3 localTarget, float turn, RouteAIPilotSettings settings)
        {
            float touchRadius = route != null ? route.TouchRadius : 6f;
            float slowRadius = Mathf.Max(settings.approachSlowRadius, touchRadius * 4f);
            float radiusBand = Mathf.Max(0.01f, slowRadius - touchRadius);
            float approach01 = 1f - Mathf.Clamp01((distance - touchRadius) / radiusBand);
            float desiredSpeed = Mathf.Lerp(settings.routeSpeed, settings.closeTargetSpeed, approach01);
            desiredSpeed = Mathf.Lerp(desiredSpeed, settings.closeTargetSpeed, Mathf.Abs(turn) * Mathf.Abs(turn));

            if (localTarget.z < 0f && distance < settings.overshootRecoveryRadius)
            {
                desiredSpeed = settings.overshootSpeed;
            }

            float speed = controller != null ? controller.CurrentSpeed : desiredSpeed;
            if (speed > desiredSpeed + settings.speedTolerance)
            {
                return settings.brakeInput;
            }
